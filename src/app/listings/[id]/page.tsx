...
export default async function StaticListingPage({ params }: PageProps) {
  const { id } = await params
  if (id === EMPTY_BUILD_ID) return <main className="marketplace-listing-page"><div className="marketplace-listing-shell rounded-3xl border border-[#3F3F46] bg-white p-8 text-center"><h1 className="text-2xl font-extrabold text-[#09090B]">No hay publicaciones disponibles en este build</h1><p className="mt-3 text-sm text-[#A1A1AA]">Esta ruta técnica no se incluye en el sitemap y solo existe para que el export estático pueda compilar sin conexión a Supabase.</p><Link href="/listings" className="marketplace-auth-primary mt-6">Explorar publicaciones</Link></div></main>
  const listing = await getBuildListing(id)
  if (!listing) notFound()
  const image = listing.media.find((item) => item.is_cover)?.url ?? listing.media[0]?.url
  const url = `${SITE_URL}/listings/${listing.id}`
  ...
  return <main className="marketplace-listing-page"><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(jsonLd) }} /><div className="marketplace-listing-shell"><Link href="/listings" className="marketplace-listing-back">← Volver a publicaciones</Link><div className="marketplace-listing-hero"><div><div className="marketplace-listing-gallery"><div className="marketplace-listing-gallery-main">{image ? <img src={image} alt={listing.title} /> : <div className="marketplace-listing-no-photo">Este vehículo todavía no tiene fotos</div>}<div className="absolute right-4 top-4"><FavoriteButton listingId={listing.id} size={21} /></div></div>{listing.media.length > 1 && <div className="grid grid-cols-4 gap-2 p-2">{listing.media.filter((item) => item.url !== image).slice(0, 4).map((item) => <img key={item.id} src={item.url} alt="" className="aspect-[4/3] w-full rounded-xl object-cover" />)}</div>}</div></div><div className="marketplace-listing-info"><p className="marketplace-eyebrow text-[#C2410C]">Publicación real · contacto directo</p><h1>{listing.title}</h1><p className="marketplace-listing-price">{priceLabel(listing)}</p><div className="mt-5 flex flex-wrap gap-2">{meta.map((item) => <span key={item} className="rounded-full bg-[#27272A] px-3 py-1.5 text-xs font-bold text-[#C2410C]">{item}</span>)}</div><div className="marketplace-listing-contact"><ContactButton listingId={listing.id} sellerId={listing.seller_id} listingTitle={listing.title} /></div><p className="mt-3 text-xs leading-relaxed text-[#A1A1AA]">Escribile al vendedor desde Sin Frenos. Tu email y teléfono no se comparten automáticamente.</p></div></div>{listing.description && <section className="marketplace-listing-panel"><h2 className="text-xl font-extrabold text-[#09090B]">Sobre este vehículo</h2><p className="mt-3 max-w-3xl whitespace-pre-line">{listing.description}</p></section>}<section className="marketplace-listing-panel flex flex-wrap gap-3 text-sm font-semibold">{listing.accepts_trade && <span>✓ Acepta permuta</span>}{listing.accepts_financing && <span>✓ Acepta financiación</span>}{listing.has_title != null && <span>✓ Documentación: {listing.has_title ? 'al día' : listing.title_status ?? 'a confirmar'}</span>}</section><section className="marketplace-listing-panel"><h2 className="text-xl font-extrabold text-[#09090B]">¿Ves un problema?</h2><p className="mt-2 text-sm text-neutral-600">Reportá datos engañosos, fotos falsas, spam u otra infracción de las reglas.</p><div className="mt-4"><ReportListingForm listingId={listing.id} /></div></section></div></main>
}
