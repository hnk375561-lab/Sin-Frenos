...
function ListingContent() {
  ...
  if (error) return <main className="marketplace-listing-page"><div className="marketplace-listing-shell rounded-3xl border border-[#FCA5A5] bg-[#FEE2E2] p-8 text-center"><h1 className="text-2xl font-extrabold text-[#09090B]">No pudimos abrir esta publicación</h1><p className="mt-2 text-sm text-[#7C2D12]">{error}</p><Link href="/listings" className="marketplace-auth-primary mt-6">Volver a publicaciones</Link></div></main>
  ...
  return <main className="marketplace-listing-page"><div className="marketplace-listing-shell"><Link href="/listings" className="marketplace-listing-back">← Volver a publicaciones</Link><div className="marketplace-listing-hero"><div><div className="marketplace-listing-gallery"><div className="marketplace-listing-gallery-main">{cover ? <img src={cover.url} alt={listing.title} /> : <div className="marketplace-listing-no-photo">Este vehículo todavía no tiene fotos</div>}<div className="absolute right-4 top-4"><FavoriteButton listingId={listing.id} size={21} /></div></div>{media.length > 1 && <div className="grid grid-cols-4 gap-2 p-2">{media.filter((m) => m.id !== cover?.id).slice(0, 4).map((m) => <img key={m.id} src={m.url} alt="" className="aspect-[4/3] w-full rounded-xl object-cover" />)}</div>}</div></div><div className="marketplace-listing-info"><p className="marketplace-eyebrow text-[#C2410C]">Publicación real · contacto directo</p><h1>{listing.title}</h1><p className="marketplace-listing-price">{price}</p><div className="mt-5 flex flex-wrap gap-2">{meta.map((item) => <span key={item} className="rounded-full bg-[#27272A] px-3 py-1.5 text-xs font-bold text-[#C2410C]">{item}</span>)}</div><div className="marketplace-listing-contact"><ContactButton listingId={listing.id} sellerId={listing.seller_id} listingTitle={listing.title} /></div><p className="mt-3 text-xs leading-relaxed text-[#A1A1AA]">Escribile al vendedor desde Sin Frenos. Tu email y teléfono no se comparten automáticamente.</p></div></div>{vehicleModel && <div className="marketplace-listing-panel">Relacionado con el catálogo técnico: <Link href={`/vehiculos/${vehicleModel.slug}`} className="font-bold text-[#C2410C]">{vehicleModel.manufacturer} {vehicleModel.title} →</Link></div>}{listing.description && <section className="marketplace-listing-panel"><h2 className="text-xl font-extrabold text-[#09090B]">Sobre este vehículo</h2><p className="mt-3 max-w-3xl whitespace-pre-line">{listing.description}</p></section>} {listing.condition_details && Object.keys(listing.condition_details).length > 0 && <section className="marketplace-listing-panel"><h2 className="text-xl font-extrabold text-[#09090B]">Lo que declara el vendedor</h2><dl className="mt-3 grid gap-2 sm:grid-cols-2">{Object.entries(listing.condition_details).map(([key, value]) => <div key={key} className="rounded-xl bg-white p-3"><dt className="text-xs font-bold uppercase text-[#A1A1AA]">{key}</dt><dd className="mt-1 text-sm text-[#09090B]">{String(value)}</dd></div>)}</dl></section>}<section className="marketplace-listing-panel flex flex-wrap gap-3 text-sm font-semibold">{listing.accepts_trade && <span>✓ Acepta permuta</span>}{listing.accepts_financing && <span>✓ Acepta financiación</span>}{listing.has_title != null && <span>✓ Documentación: {listing.has_title ? 'al día' : listing.title_status ?? 'a confirmar'}</span>}</section></div></main>
}
