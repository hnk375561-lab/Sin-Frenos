...
function joinSpecs(values: unknown[], separator = ' · '): string {
  return values.map((value) => displaySpec(value, '')).filter(Boolean).join(separator) || 'No documentado'
}
...
export default async function Home() {
  ...
  const featuredVehicle = vehicles.find((vehicle) => vehicle.slug === 'tesla-model-3') ?? featured[0] ?? vehicles.find((vehicle) => vehicle.featured) ?? vehicles[0]
  const featuredImage = featuredVehicle ? imageBySlug[`vehiculos/${featuredVehicle.slug}`] : null
  const featuredSpecs = featuredVehicle ? [
    { label: 'Autonomía', value: featuredVehicle.autonomia, icon: '↯', primary: true },
    { label: '0–100 km/h', value: featuredVehicle.performance?.acceleration, icon: '◷', primary: true },
    { label: 'Velocidad máx.', value: featuredVehicle.performance?.speed, icon: '↗', primary: true },
    { label: 'Batería', value: `${displaySpec(featuredVehicle.tipoBateria)} · ${displaySpec(featuredVehicle.capacidadBateria)}`, icon: '⚡', primary: false },
    { label: 'Consumo', value: featuredVehicle.consumo, icon: '⌁', primary: false },
    { label: 'Dimensiones', value: featuredVehicle.dimensiones, icon: '↔', primary: false },
    { label: 'Peso', value: featuredVehicle.peso, icon: '◈', primary: false },
    { label: 'Motor', value: joinSpecs([featuredVehicle.tipoMotor, featuredVehicle.especificacionesMotor?.potenciaKW, featuredVehicle.especificacionesMotor?.torque]), icon: '⚙', primary: false },
    { label: 'Seguridad Euro NCAP', value: joinSpecs([featuredVehicle.safety?.euroNCAP, featuredVehicle.safety?.puntaje && `${featuredVehicle.safety.puntaje} puntos`, featuredVehicle.safety?.proteccionAdultos && `Adultos ${featuredVehicle.safety.proteccionAdultos}%`, featuredVehicle.safety?.asistenciaSistemas && `Asistencia ${featuredVehicle.safety.asistenciaSistemas}%`]), icon: '✦', primary: false },
    { label: 'Equipamiento', value: joinSpecs(featuredVehicle.equipamiento ?? [], ', '), icon: '＋', primary: false },
    { label: 'Performance', value: joinSpecs([featuredVehicle.performanceData?.aceleracion?.['0-100kmh'], featuredVehicle.performanceData?.velocidad?.maxima && `${featuredVehicle.performanceData.velocidad.maxima} km/h máx.`, featuredVehicle.performanceData?.consumo?.mixto, featuredVehicle.performanceData?.autonomia?.real && `${featuredVehicle.performanceData.autonomia.real} real`]), icon: '◌', primary: false },
    { label: 'Variantes', value: joinSpecs((featuredVehicle.variants ?? []).map((variant) => `${displaySpec(variant.nombre)}: ${displaySpec(variant.potencia)} · ${displaySpec(variant.autonomia)}`), ' / '), icon: '▦', primary: false },
    { label: 'Habitabilidad', value: joinSpecs([featuredVehicle.asientos && `${featuredVehicle.asientos} asientos`, featuredVehicle.baul && `Baúl ${featuredVehicle.baul}`]), icon: '□', primary: false },
  ] : []
  const premiumVehicles = vehicles
    .filter((vehicle) => vehicle.homepageCurated === 'premium')
    .sort((a, b) => (a.homepageCuratedOrder ?? Number.MAX_SAFE_INTEGER) - (b.homepageCuratedOrder ?? Number.MAX_SAFE_INTEGER))
  ...
  return (
    <>
      ...
      <main className="min-h-screen bg-surface-page">
        ...
        {featuredVehicle && <section className="border-b border-edge py-16 sm:py-24" aria-labelledby="featured-heading"><div className="container-max"><Reveal direction="chapter"><p className="eyebrow text-auto-accent">04 · Una ficha para mirar en profundidad</p><p className="mt-3 max-w-2xl text-body">Tesla Model 3 2023 RWD: datos de desempeño, motor, seguridad, equipamiento, variantes y habitabilidad tomados de la ficha técnica existente.</p><div className="mt-8 grid gap-8 lg:grid-cols-[1.1fr_.9fr] lg:items-start"><div className="overflow-hidden rounded-2xl border border-edge bg-surface-card"><EntityImage entity={featuredVehicle} image={featuredImage} variant="portrait" priority /></div><div><h2 id="featured-heading" className="text-4xl font-bold tracking-tight text-strong sm:text-6xl">{featuredVehicle.title}</h2><p className="mt-4 text-lg leading-relaxed text-body">{featuredVehicle.description}</p><div className="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-3">{featuredSpecs.map(({ label, value, icon, primary }) => <div key={label} className={primary ? 'rounded-xl border border-auto-accent/40 bg-auto-accent/5 p-4 sm:p-5' : 'rounded-lg border border-edge bg-surface-card p-3'}><span className={primary ? 'block font-mono text-[10px] uppercase tracking-wider text-auto-accent' : 'block font-mono text-[10px] uppercase tracking-wider text-muted'}><span aria-hidden="true" className="mr-1">{icon}</span>{label}</span><span className={primary ? 'mt-2 block text-base font-bold leading-tight text-strong sm:text-lg' : 'mt-1 block text-sm font-medium leading-snug text-muted'}>{displaySpec(value)}</span></div>)}</div><Link href={`/vehiculos/${featuredVehicle.slug}`} className="mt-7 inline-flex min-h-11 items-center rounded-full bg-auto-accent px-5 py-3 font-semibold text-white transition-[transform,background-color] duration-200 hover:bg-auto-accent-strong hover:-translate-y-0.5 active:scale-[.98]">Abrir ficha completa →</Link></div></div></Reveal></div></section>}

        <section className="border-b border-edge bg-surface-card py-16 sm:py-20" aria-labelledby="featured-grid-heading"><div className="container-max"><div className="flex flex-wrap items-end justify-between gap-5"><div><p className="eyebrow text-auto-accent">05 · Curaduría premium alternativa</p><h2 id="featured-grid-heading" className="mt-3 text-3xl font-bold text-strong sm:text-5xl">Potencia, diseño y carácter.</h2><p className="mt-3 max-w-2xl text-body">Selección editorial fija de modelos premium con fotografía verificada y precios USD de referencia. La sección 3 es dinámica y ordena el archivo por dato; esta propone cuatro alternativas para mirar.</p></div><Link href="/vehiculos" className="link-underline font-semibold text-auto-accent">Todo el catálogo →</Link></div><div className="mt-8 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">{premiumVehicles.map((vehicle, index) => <Reveal key={vehicle.slug} index={index}><article className="group overflow-hidden rounded-2xl border border-edge bg-surface-page"><Link href={`/vehiculos/${vehicle.slug}`} className="block"><EntityImage entity={vehicle} image={imageBySlug[`vehiculos/${vehicle.slug}`]} variant="thumbnail" priority={index < 2} /><div className="p-4"><div className="flex items-start justify-between gap-3"><div><p className="font-mono text-[10px] uppercase tracking-[.16em] text-auto-accent">{vehicle.manufacturer}</p><h3 className="mt-1 text-xl font-bold text-strong">{vehicle.title}</h3></div><span className="text-lg text-auto-accent transition-transform duration-200 group-hover:translate-x-1">↗</span></div><p className="mt-2 text-sm font-semibold text-body">{displaySpec(vehicle.price)}</p><dl className="mt-4 grid grid-cols-2 gap-2 border-t border-edge pt-3">{[["Potencia", vehicle.power], ["0–100", vehicle.performance?.acceleration], ["Máxima", vehicle.performance?.speed], ["Motor", vehicle.tipoMotor], ["Torque", vehicle.especificacionesMotor?.torque], ["Tracción", vehicle.traccion], ["Transmisión", vehicle.transmision], ["Peso", vehicle.peso]].map(([label, value]) => <div key={label}><dt className="font-mono text-[9px] uppercase tracking-wider text-muted">{label}</dt><dd className="mt-1 line-clamp-2 text-xs font-medium text-strong">{displaySpec(value)}</dd></div>)}</dl></div></Link></article></Reveal>)}</div></div></section>
      </main>
    </>
  )
}
