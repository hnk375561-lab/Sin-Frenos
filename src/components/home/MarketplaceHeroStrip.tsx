...
function PlaceholderCard({ index }: { index: number }) {
  return (
    <div className="marketplace-placeholder-card" aria-label="Espacio reservado para una futura publicación">
      ...
      <div className="p-5">
        <p className="text-xs font-bold uppercase tracking-[0.16em] text-[#0b7a75]">Próxima publicación</p>
        <p className="mt-3 text-lg font-semibold tracking-[-0.03em] text-[#12212a]">Tu vehículo puede ocupar este lugar.</p>
        <p className="mt-2 text-sm leading-relaxed text-[#667780]">Este espacio está reservado para una oferta real de la comunidad.</p>
        <Link href="/publicar" prefetch={false} className="mt-5 inline-flex text-sm font-bold text-[#f05a3c] hover:text-[#c8442a]">Publicar ahora →</Link>
      </div>
    </div>
  )
}
    ...
    <section className="marketplace-listings-hero" aria-labelledby="recent-listings-heading">
      <div className="marketplace-home-container">
        <div className="flex flex-wrap items-end justify-between gap-5">
          <div>
            <p className="marketplace-eyebrow text-[#82d4ce]">Marketplace Sin Frenos</p>
        ...
        <div className="mt-8 flex flex-wrap items-center justify-between gap-4 border-t border-white/10 pt-5">
          ...
          <Link href="/listings" className="text-sm font-bold text-[#82d4ce] transition hover:text-white">Explorar marketplace →</Link>
        </div>
      </div>
    </section>
  )
